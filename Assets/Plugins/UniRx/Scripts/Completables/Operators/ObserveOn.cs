...
namespace UniRx.Completables.Operators
{
    internal class ObserveOnCompletable : OperatorCompletableBase
    {
        ...
        private class ObserveOnObserver : OperatorCompletableObserverBase
        {
            ...
            public ObserveOnObserver(ObserveOnCompletable parent, ICompletableObserver observer, IDisposable cancel) : base(observer, cancel)
            {
                this.parent = parent;
            }
            ...
            {
                QueueAction(new Notification<Unit>.OnErrorNotification(error));
            }

            public override void OnCompleted()
            {
                QueueAction(new Notification<Unit>.OnCompletedNotification());
            }
        ...
        private class ScheduledObserveOnObserver : OperatorCompletableObserverBase
        {
            ...
            public ScheduledObserveOnObserver(ObserveOnCompletable parent, ISchedulerQueueing scheduler, ICompletableObserver observer, IDisposable cancel) : base(observer, cancel)
            {
                this.parent = parent;
                this.scheduler = scheduler;
            ...
            public override void OnError(Exception error)
            {
                scheduler.ScheduleQueueing(isDisposed, error, ex =>
                {
            ...
            public override void OnCompleted()
            {
                scheduler.ScheduleQueueing(isDisposed, Unit.Default, _ =>
                {
