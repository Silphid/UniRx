...
namespace UniRx.Completables
{
    public sealed class CompletableSubject : ICompletableSubject, IDisposable, IOptimizedCompletable
    {
        ...
        public bool HasObservers
        {
            get
            {
                return !(outObserver is EmptyCompletableObserver) && !isStopped && !isDisposed;
            }
        }
            ...
            lock (observerLock)
            {
                ...
                if (!isStopped)
                {
                    ...
                    else
                    {
                        ...
                        {
                            outObserver = new ListCompletableObserver(new ImmutableList<ICompletableObserver>(new[] { current, observer }));
                        }
                    }
